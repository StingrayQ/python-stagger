...
class StaggerDatabase:
    ...
    def get_track_by_id(self, track_id, user_id):
        ...
        try:
            ...
            return None
        # delete track by user
    def delete_track(self, user_id, track_id):
        ...
        try:
            ...
            cursor.execute('DELETE FROM tracks WHERE id = ? AND user_id = ?', (track_id, user_id))

            if cursor.rowcount > 0:
                conn.commit()
                return True, "Track deleted successfully!"
            else:
                return False, "Track not found or access denied!"

        except Exception as e:
            return False, f"Error deleting track: {e}"
        finally:
            if conn:
                try:
                    conn.close()
    ...
    def update_track(self, track_id, user_id, track_name, corner_length_ft, banking_degrees, surface_type):
        ...
        try:
            ...
            cursor.execute('''
                           ...
                           WHERE id = ?
                             AND user_id = ?
                           ''',
                           (track_name, corner_length_ft, banking_degrees, surface_type, track_id, user_id))

            if cursor.rowcount > 0:
                conn.commit()
                conn.close()
                return True, f"Track '{track_name}' updated successfully!"
            else:
                conn.close()
                return False, "Track not found or access denied!"

        except sqlite3.IntegrityError as e:
            if "track_name" in str(e).lower():
                return False, "Track name already exists!"
            else:
                return False, f"Database constraint error: {e}"
        except Exception as e:
            return False, f"Error updating track: {e}"
